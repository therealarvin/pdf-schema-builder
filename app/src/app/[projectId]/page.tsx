...
import { PDFField, Schema, FieldGroup, SchemaItem } from "@/types/schema";
  ...
  const handleFieldClick = (field: PDFField, event?: React.MouseEvent) => {
    ...
    if (visibilitySelectionMode) {
      ...
      if (targetSchemaItem) {
        ...
        if (editingItem && targetSchemaItem) {
          const updatedSchema = schema.map(item => {
            if (item.unique_id === visibilitySelectionMode.schemaItemId) {
              ...
              if (visibleIf[visibilitySelectionMode.conditionIndex] && targetSchemaItem) {
